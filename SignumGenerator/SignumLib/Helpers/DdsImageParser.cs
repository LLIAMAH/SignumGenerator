...
namespace SignumLib.Helpers
{
    ...
    public class DDSImage : IDisposable
    {
        ...
        {
            if (ddsImage is not {CanRead: true}) return;
        ...
        private void Parse(BinaryReader reader)
        {
            ...
            if (ReadHeader(reader, ref header))
            {
                ...
                if (data != null)
                {
                    var rawData = this.DecompressData(header, data, pixelFormat);
                    this.m_bitmap = CreateBitmap((int)header.width, (int)header.height, rawData);
                }
            }
        }
        ...
        private static System.Drawing.Bitmap CreateBitmap(int width, int height, byte[] rawData)
        {
        ...
        private static PixelFormat GetFormat(DdsStruct header, ref uint blocksize)
        {
            ...
            {
                // uncompressed image
                format = (header.pixelFormat.flags & DDPF_LUMINANCE) == DDPF_LUMINANCE
                    ? (header.pixelFormat.flags & DDPF_ALPHAPIXELS) == DDPF_ALPHAPIXELS ? PixelFormat.LUMINANCE_ALPHA :
                    PixelFormat.LUMINANCE
                    : (header.pixelFormat.flags & DDPF_ALPHAPIXELS) == DDPF_ALPHAPIXELS
                        ? PixelFormat.RGBA
                        : PixelFormat.RGB;
                //// uncompressed image
            ...
            fixed (byte* bytePtr = data)
            {
                ...
                for (var z = 0; z < depth; z++)
                {
                    for (var y = 0; y < height; y += 4)
                    {
                        ...
                        {
                            var color0 = *((ushort*)temp);
                            var color1 = *((ushort*)(temp + 2));
        ...
        private unsafe byte[] DecompressRGB(DdsStruct header, byte[] data, PixelFormat pixelFormat)
        {
            // allocate bitmap
            var bpp = (int)(PixelFormatToBpp(pixelFormat, header.pixelFormat.rgbBitCount));
            var bps = (int)(header.width * bpp * this.PixelFormatToBpc(pixelFormat));
            var sizeOfPlane = (int)(bps * header.height);
            var width = (int)header.width;
            var height = (int)header.height;
            var depth = (int)header.depth;
                                    ...
                                    if ((y + k + j) < height)
                                    {
                                        for (var i = 0; i < 4; i++)
                                        {
                                            ...
                                            if (((x + i) < width))
                                            {
                                                ...
                                                var t = 127 * 128 - (tx - 127) * (tx - 128) - (ty - 127) * (ty - 128);
                                                if (t > 0)
        ...
        private unsafe byte[] DecompressARGB(DdsStruct header, byte[] data, PixelFormat pixelFormat)
        {
            ...
            var tempBpp = header.pixelFormat.rgbBitCount / 8;
        ...
        /// </summary>
        public System.Drawing.Bitmap BitmapImage => this.m_bitmap;
